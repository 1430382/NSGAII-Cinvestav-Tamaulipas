...
float tock(){
   ...
   return ((float)(end.tv_sec-start.tv_sec)*1000000 + (float)(end.tv_usec-start.tv_usec)) / 1000000 ;
	 //finishTime = clock();
	 //return(double)(finishTime-startTime)/(double)CLOCKS_PER_SEC;
}
...
{
char aux;
	fscanf(input, "%d %d %d %lf" ,&N, &L, &calc, &calc1);
	taxa = (Taxon *)malloc(N * sizeof(Taxon));
	for (auto i = 0; i < N; i++) {
	    taxa[i].symbols = (char *)malloc(L * sizeof(char));
	    taxa[i].values = (int *)malloc(L * sizeof(int));
...
int main(int argc, char **argv) {
  ...
  double elapsed = (double)(stopclock - startclock) * 1000.0 / CLOCKS_PER_SEC;
 	int i,j;
 	popsize = 100;
	ngen = 500;
	ncon = 0;
	...
	//report_best(parent_pop, stdout);
 	...
 	free(is);
 	free(randarr);
	fprintf(stderr,"Time elapsed in ms: %f\n", elapsed);
 	return(0);
}
void deallocate_taxa()
{
