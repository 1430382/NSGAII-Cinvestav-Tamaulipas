...
double evaluateTreeAs(Node *n, int M) {
  ...
  aux += parsimony(n->left->values, n->right->values);
   ...
   aux+=jaccard(n->left->values, n->right->values);
  }
  else if(M == 2){
  ...
  aux+=manhattan(n->left->values, n->right->values);   
  }
  else if (M==7) {
  aux += (-1) * bitj(n->left->values, n->right->values);
  }
...
void newRandomTree(individual *ind, Taxon *taxa) {
  auto z, n1, n2, cN = N;
  auto *existingNodes = (int *)malloc(N * sizeof(int));
  for (z = 0; z < N; z++) {
...
void treeFromNewick(individual *T, Taxon *Taxa, char *newick){	
  char *reader = newick;
  long val;
  auto currentNode = N, nextNode = N+1, i,z;
